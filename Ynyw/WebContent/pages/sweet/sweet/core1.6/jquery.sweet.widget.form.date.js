    ...
    $.widget("sweet.widgetFormDate", $.sweet.widgetFormInput, /** @lends Sweet.form.Date.prototype*/{
            ...
            // 时间
                ...
                var emElClass = "sweet-form-date-float-south-time-em",
                    ...
                    inputSecondEl = me.inputSecondEl = $("<input>").attr(inputAttrs).keydown(function() {return false;})
                        .addClass(inputClass)
                        .addClass(inputSouthClass),
                    emEl1 = $("<em>").addClass(emElClass).html(Sweet.constants.symbol.COLON),
                    emEl2 = $("<em>").addClass(emElClass).html(Sweet.constants.symbol.COLON),
                    floatHourPanelEl,
                    floatMinutePanelEl,
                    floatSecondEl,
                    panelObjs = [];
        ...
        _createMonthPanel: function(me) {
            ...
            for (var i = 0, j = 0; i < 12; i++, j++) {
                ...
                monthAEl[j] = $("<a>").val(i + 1).html(Sweet.core.i18n.month[i]);
        ...
        _changeMonthPanel: function(me, month, inputEl) {
            ...
            for (var i = 0, j = 0; i < 12; i++, j++) {
                ...
                me.monthAEl[j].val(i + 1).html(Sweet.core.i18n.month[i]);
            }
        },
        /**
         * @private 
        ...
        _changeDateMidContent: function(me) {
            ...
            for (var i = 0, j = 0, k = 0; i < dayGridNum; i++) {
                ...
                if (i < beginDay) {
                    ...
                    if (!opt.showDays) {
                        dateEmEl[i].html(days[i]).val(days[i]);
                        if(invalidTestResult){
                        ...
                        dateEmEl[i].html("").val(days[i]).removeClass(dayInvalid).addClass(invalidCls);
                    }
        ...
        _createTimePanel: function(me, arr, inputObj) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                timeAEl[i] = $("<a>").val(arr[i]).html(arr[i]).appendTo(timeLiEl);
            }
            timeUlEl.appendTo(timeDivEl);
        ...
        _changeTimePanel: function(me, time, inputEl, spinnerTimeTypes) {
            ...
            function selectArr(arr) {
                for (var i = 0; i < arr.length; i++) {
                    ...
                    me.timeAEl[i].val(arr[i]).html(arr[i]);
                }
            }
        },
        /**
        ...
        _createDateSouthItem: function() {
            var me = this,
                ...
                okButtonEl = me.okButtonEl = $("<button>").addClass(okButtonElClass).html(i18nObj.okButton).appendTo(spanEl),
                labelEl = $("<label>").html(i18nObj.time + COLON).addClass(southLabelClass).appendTo(southItemEl),
