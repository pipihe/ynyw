...
(function($, undefined) {
    ...
    $.widget("sweet.widgetTreeTree_v1", $.sweet.widgetTree, /** @lends Sweet.tree.Tree_v1.prototype */{
        ...
        setValue: function(value) {
            ...
            me._reset();
            //解析value数据
            me.parseDatas(optValue, valueListDatas, valueObjDatas, "", false, null);
            //如果为单选
            if (!me.options.multi) {
        ...
        _doLayout: function() {
            ...
            if (!isNaN(parseInt(me.options.height))) {
                height = parseInt(me.options.height);
            }
            if (!isNaN(parseInt(me.options.width))) {
                width = parseInt(me.options.width);
            }
         ...
         */
        getValue: function(valueKeys) {
            var me = this;
            return me._getValue(valueKeys);
        },
        /**
         * @private
        ...
        _getValue: function(valueKeys) {
            ...
            //从当前操作的数据中还原选中数据
            me.parseTreeDatas(me.selectedDatas, me.optObjDatas, keys, result);
            if (!result["data"]) {
                return [];
            }
        ...
        _doRenderAll: function(datas) {
            ...
            for (var i = 0; i < datas.length; i++) {
                ...
                text = $.htmlEscape($.nullToString(tNode[KEY_TEXT]));
                textEl = $("<span>").addClass(_treeNodeTextClass).html(text).appendTo(liEl);
        ...
        _addLiNode: function(node, top) {
            ...
            text = $.htmlEscape($.nullToString(node[KEY_TEXT]));
            textEl = $("<span>").html(text).appendTo(liEl);
        ...
        _doAddRootNode: function(cNode, node, isBefore) {
            ...
            text = $.htmlEscape($.nullToString(cNode[KEY_TEXT]));
            cTextEl = $("<span>").addClass(_treeNodeTextClass).html(text).appendTo(cLiEl);
            // 是否有tip提示
            if (me.options.tip) {
        ...
        _doSelected: function(node) {
            ...
            if (node && !node[KEY_LEAF] && !(true === me.options.editable || "true" === me.options.editable || $.isArray(me.options.editable))) {
                return;
            }
            if (!$.isEmptyObject(me.selectedDatas)) {
                for (var key in me.selectedDatas) {
        ...
        insertAfter: function(node1, node2) {
            ...
            text = $.htmlEscape($.nullToString(node1[KEY_TEXT]));
            cTextEl = $("<span>").addClass(_treeNodeTextClass).html(text).appendTo(cLiEl);
            // 是否有tip提示
            if (me.options.tip) {
        ...
        _addCNodeCount: function(node, liEl, textEl) {
            ...
            nTextEl = $("<span>").addClass(treeNodeCCTextClass).html(nText).appendTo(liEl);
            // 是否有tip提示
            if (me.options.tip) {
