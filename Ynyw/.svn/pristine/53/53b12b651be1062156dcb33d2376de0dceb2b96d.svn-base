...
(function($, undefined) {
    ...
    $.widget("sweet.widgetChartTiles", $.sweet.widgetChart, /** @lends Sweet.chart.Tiles.prototype*/{
		...
		_calcText : function(ctx, txt, font, width){
            ...
            ctx.save();
            ctx.font = font;
            //var suffixWidth = ctx.measureText(suffix).width;
            while(true){
        ...
        _doLayout: function() {
            ...
            // 动态调整Value的字体大小
            //values = this.listEl.find("." + valueClass);
            //units = this.listEl.find("." + unitClass);

            outerWidth = this.tilesEl.width();
            outerHeight = this.tilesEl.height();
            //width = _getMaxWidth();
            height = this.listEl.height();
            //step = (width > outerWidth || height > outerHeight) ? -1 : 1;
            /*
            font = parseInt(values.css("font-size"), 10);
            while ((step < 0 && font > minFont) || (step > 0 && font < maxFont)) {
                font += step;

                values.css("font-size", font + "px");
                units.css("font-size", Math.floor(font * ratio) + "px");

                width = _getMaxWidth();
                height = this.listEl.height();

                if (step > 0 && (width >= outerWidth || height >= outerHeight)) {
                    font--;
                    values.css("font-size", font + "px");
                    units.css("font-size", Math.floor(font * ratio) + "px");
                    break;
                }
                else if (step < 0 && width <= outerWidth && height <= outerHeight) {
                    break;
                }
            }
            */
            ...
            margin = outerHeight - height - 3;
            units.css("margin-top", (margin > 2 ? margin : 2) + "px");
            */
