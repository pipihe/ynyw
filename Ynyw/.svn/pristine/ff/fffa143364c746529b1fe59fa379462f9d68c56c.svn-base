...
(function($, undefined) {
    ...
    $.widget("sweet.widgetScoreCard", $.sweet.widget, /**lends Sweet.ScoreCard.prototype*/{
        ...
        _createSweetWidget: function() {
            var me = this;
            me.cardEl = $("<div>").addClass(cardClass).attr("id", me.options.id);
            //创建记分卡组件
            me._createScoreCard();
        },
        /**
        ...
        _createScoreCard: function() {
            ...
            if (txtPos === "up" && $.isNotNull(options.text)) {
                me.titleEl = $("<span>").appendTo(me.cardEl)
                        .text(options.text)
        ...
        _doLayout: function() {
            ...
            //动态设定组件高度
            if ($.isNotNull(options.value.text)) {
                titleH2 = me.titleEl.height();
            }
            if (me.offsetEl) {
                offsetH = me.offsetEl.height();
            }
            if (me.valueEl) {
                valueH = me.valueEl.height();
            }
